...
if continuum:
    ...
    for file in os.listdir(RENDER_CACHE_1):
        if file.endswith('.exr') and os.path.getsize(os.path.join(RENDER_CACHE_1, file)) > 104_857_600:  # 100 MB
...
if continuum:
    ...
    if os.path.isfile(yml):
        render_id = int(list(quick_yaml.load(open(yml, 'r').read()).keys())[-1]) + 1
    else:
        render_id = 1
